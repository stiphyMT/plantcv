...
def rotate(img, rotation_deg, crop):
    ...
    if params.debug == pcvc.DEBUG_PRINT:
        print_image( rotated_img, os.path.join(params.debug_outdir, "{0}{1}_rotated_img.png".format(  params.device, rotation_deg)))

    elif params.debug == pcvc.DEBUG_PLOT:
        if len( np.shape( img)) == 3:
            plot_image( rotated_img)
        else:
            plot_image( rotated_img, cmap = pcvc.COLOUR_MAP_GREY)
