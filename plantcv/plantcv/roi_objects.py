...
def roi_objects( img, roi_type, roi_contour, roi_hierarchy, object_contour, obj_hierarchy):
    ...
    roi_type       = 'cutto' or 'partial' (for partially inside), or 'massc' (for center of mass inside)
    ...
    # Cuts off objects that do not have their center of mass inside the ROI
        ...
        fatal_error('ROI Type {} is not "cutto", "partial", or "massc"!'.format( roi_type))

    if params.debug == pcvc.DEBUG_PRINT:
        print_image( w_back, ( str( params.device) + '_roi_objects.png'))
        print_image( ori_img, ( str( params.device) + '_obj_on_img.png'))
        print_image( mask, ( str( params.device) + '_roi_mask.png'))
