...
def pseudocolor(gray_img, obj = None, mask = None, cmap = None, background = pcvc.VISUALIZE_BACKGROUND_GREY, min_value = 0, max_value = 255,
    ...
    if mask is not None:
        if obj is not None:
            ...
            if background.upper() == pcvc.VISUALIZE_BACKGROUND_GREY:
        ...
        if background.upper() == pcvc.VISUALIZE_BACKGROUND_BLACK:
            # Background is all zeros
            bkg_img = np.zeros(np.shape(gray_img1), dtype=np.uint8)
            # Use the gray cmap for the background
            bkg_cmap = "gray"
        elif background.upper() == pcvc.VISUALIZE_BACKGROUND_WHITE:
            # Background is all 255 (white)
            bkg_img = np.zeros(np.shape(gray_img1), dtype=np.uint8)
            bkg_img += 255
            bkg_cmap = "gray"
        elif background.upper() == pcvc.VISUALIZE_BACKGROUND_GREY:
