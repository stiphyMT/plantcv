...
def find_color_card(rgb_img, threshold_type =  pcvc.TRANSFORM_FIND_CC_AGAUSS, threshvalue = 125, blurry = False, background = pcvc.TRANSFORM_BACKGROUND_DARK):
    ...
    if background == pcvc.TRANSFORM_BACKGROUND_LIGHT:
    ...
    elif background != pcvc.TRANSFORM_BACKGROUND_DARK:
    ...
    elif threshold_type == pcvc.TRANSFORM_FIND_CC_NORMAL:
        # Blur slightly so defects on card squares and background patterns are less likely to be picked up
        gaussian = cv2.GaussianBlur(gray_img, (5, 5), 0)
        ret, threshold = cv2.threshold(gaussian, threshvalue, 255, cv2.THRESH_BINARY)
    elif threshold_type == pcvc.TRANSFORM_FIND_CC_AGAUSS:
        # Blur slightly so defects on card squares and background patterns are less likely to be picked up
