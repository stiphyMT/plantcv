...
def shift_img( img, number, side = pcvc.SHIFT_IMG_RIGHT):
    ...
    if params.debug == pcvc.DEBUG_PRINT:
        print_image( adjusted_img, ( str( params.device) + "_shifted_img.png"))
    elif params.debug == pcvc.DEBUG_PLOT:
        if len(np.shape( adjusted_img)) == 3:
            plot_image( adjusted_img)
        else:
            plot_image( adjusted_img, cmap = pcvc.COLOUR_MAP_GREY)
