...
def gaussian( gray_img, max_value, object_type = pcvc.THRESHOLD_OBJ_LIGHT):
...
def mean( gray_img, max_value, object_type = pcvc.THRESHOLD_OBJ_LIGHT):
...
# Otsu autothreshold
def otsu( gray_img, max_value, object_type = pcvc.THRESHOLD_OBJ_LIGHT):
...
def triangle( gray_img, max_value, object_type = pcvc.THRESHOLD_OBJ_LIGHT, xstep = 1):
    ...
    if params.debug is not None:
        if params.debug == 'print':
            ...
            fig_name_hist = os.path.join(params.debug_outdir,
                                         "{0}_triangle_thresh_hist_{1}.png".format( params.device, autothreshval))
...
def _call_threshold( gray_img, threshold, max_value, threshold_method, method_name):
    ...
    if params.debug == pcvc.DEBUG_PRINT:
        print_image( bin_img, os.path.join(params.debug_outdir, "{0}_method_name{1}.png".format( params.device, threshold)))
...
def _call_adaptive_threshold( gray_img, max_value, adaptive_method, threshold_method, method_name):
    ...
    if params.debug == pcvc.DEBUG_PRINT:
        print_image( bin_img, os.path.join(params.debug_outdir,
                                          "{0}_{1}.png".format( params.device, method_name)))
    elif params.debug == pcvc.DEBUG_PLOT:
        plot_image( bin_img, cmap = 'gray')
...
def _plot( x, mph, mpd, threshold, edge, valley, ax, ind):
    ...
    if ind.size:
        ...
        ax.plot( ind, x[ind], '+', mfc = None, mec = 'r', mew = 2, ms = 8,
                label = '%d %s'.format( ind.size, label))
    ...
    ax.set_title( "%s (mph = %s, mpd = %d, threshold = %s, edge = '%s')".format(mode, mph, mpd, threshold, edge))
    # plt.grid()
    plt.show()
