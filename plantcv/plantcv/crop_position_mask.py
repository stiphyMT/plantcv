...
def crop_position_mask( img, mask, x, y, v_pos = "top", h_pos = "right"):
    ...
    if v_pos == "bottom":
        ...
        # print_image( maskv,( str( params.device) + "_push-bottom-test.png"))
        ...
        if mx < ix:
            ...
            if r % 2 == 0:
                ...
                maskv = np.vstack(( rows1, maskv, rows2))
        if params.debug == pcvc.DEBUG_PRINT:
            print_image( maskv, ( str( params.device) + "_push-bottom.png"))
        elif params.debug == pcvc.DEBUG_PLOT:
            plot_image( maskv, cmap = pcvc.COLOR_MAP_GRAY)
    ...
    if params.debug is not None:
        if params.debug == pcvc.DEBUG_PRINT:
            ...
            plot_image( newmask, cmap = pcvc.COLOR_MAP_GRAY)
        ...
        if params.debug == pcvc.DEBUG_PRINT:
            print_image( ori_img, ( str( params.device) + '_mask_overlay.png'))
        elif params.debug == pcvc.DEBUG_PLOT:
            plot_image( ori_img)
